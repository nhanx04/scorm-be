...
public class ScormManifestGenerator {
    ...
    public String generateManifest(ScormPackage scormPackage) {
        try {
            ...
            // Organizations
            addOrganizations(manifest, scormPackage);

            // Resources
            addResources(manifest, scormPackage);
    ...
    private void addOrganizations(Element manifest, ScormPackage scormPackage) {
        Element organizations = manifest.addElement("organizations");
        organizations.addAttribute("default", "ORG-" + scormPackage.getId());

        Element organization = organizations.addElement("organization");
        organization.addAttribute("identifier", "ORG-" + scormPackage.getId());
        organization.addAttribute("adlseq:objectivesGlobalToSystem", "false");

        organization.addElement("title").setText(scormPackage.getTitle());

        // Add item for the quiz
        Element item = organization.addElement("item");
        item.addAttribute("identifier", "ITEM-" + scormPackage.getId());
        item.addAttribute("identifierref", "RES-" + scormPackage.getId());
        item.addAttribute("isvisible", "true");

        item.addElement("title").setText(scormPackage.getTitle());
    ...
    private void addResources(Element manifest, ScormPackage scormPackage) {
        Element resources = manifest.addElement("resources");

        Element resource = resources.addElement("resource");
        resource.addAttribute("identifier", "RES-" + scormPackage.getId());
